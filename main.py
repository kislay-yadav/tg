...
RENDER_URL   = os.environ.get("RENDER_EXTERNAL_URL", "https://tg-n7dh.onrender.com")
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .timer-bar{height:4px;border-radius:8px;background:linear-gradient(90deg,#54a0ff,#a29bfe);width:100%;transition:width 1s linear}
  .timer-text{font-size:13px;color:#74b9ff;text-align:center;margin-top:8px;font-weight:600}
  .resend-btn{color:#74b9ff;cursor:pointer;font-size:13px;text-align:center;
              margin-top:12px;display:none;background:rgba(84,160,255,0.1);
              border:1px solid rgba(84,160,255,0.2);padding:10px 16px;
              border-radius:10px;width:100%;font-weight:600}
...
<div class="card">
  ...
  <div class="step active" id="step1">
    ...
    <p class="hint">Enter with country code · Example: +91, +1, +44</p>
  </div>
  ...
  <div class="step" id="step2">
    ...
    <p class="hint">Check your Telegram app for the code · Expires in 2 minutes</p>
  </div>
  ...
  <div class="step" id="step3">
    ...
    <p class="hint">Two-Step Verification password from Telegram</p>
  </div>
  ...
  <div class="step" id="step4">
    <div class="success-box">
      ...
      <div class="success-sub">Session saved.<br>Go back to Telegram bot → /start</div>
      <div class="divider"></div>
      <div style="font-size:13px;color:rgba(255,255,255,0.3)">You can close this page</div>
    </div>
  </div>
</div>
<script>
let timerSecs=120,timerInterval=null;
function showMsg(id,text,type){const e=document.getElementById(id);e.className='msg '+type;e.textContent=text;e.style.display='block'}
function hideMsg(id){document.getElementById(id).style.display='none'}
function setLoading(id,loading,text){const b=document.getElementById(id);b.disabled=loading;b.innerHTML=loading?'<span class="spinner"></span>Please wait…':text}
function goStep(n){
  document.querySelectorAll('.step').forEach((e,i)=>e.classList.toggle('active',i+1===n));
  ['d1','d2','d3'].forEach((id,i)=>{const d=document.getElementById(id);d.className='dot'+(i+1<n?' done':i+1===n?' active':'')});
}
function startTimer(){
  timerSecs=120;
  const bar=document.getElementById('timerBar'),text=document.getElementById('timerText'),resend=document.getElementById('resendBtn');
  resend.style.display='none';clearInterval(timerInterval);
  timerInterval=setInterval(()=>{
    timerSecs--;bar.style.width=(timerSecs/120*100)+'%';
    if(timerSecs<=0){clearInterval(timerInterval);text.textContent='⏰ Expired — click Resend';text.style.color='#ff6b81';bar.style.background='#ff6b81';resend.style.display='block'}
    else{text.textContent='⏳ '+timerSecs+'s remaining';text.style.color=timerSecs<30?'#feca57':'#74b9ff'}
  },1000);
}
async function sendOTP(){
  const phone=document.getElementById('phone').value.trim();
  if(!phone){showMsg('msg1','⚠️ Enter your phone number','error');return}
  setLoading('btn1',true,'Send OTP →');hideMsg('msg1');
  try{
    const r=await fetch('/send_otp',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone})});
    const d=await r.json();
    if(d.ok){goStep(2);startTimer()}
    else{showMsg('msg1','❌ '+(d.error||'Failed to send OTP'),'error');setLoading('btn1',false,'Send OTP →')}
  }catch(e){showMsg('msg1','❌ Network error. Try again.','error');setLoading('btn1',false,'Send OTP →')}
}
async function verifyOTP(){
  const otp=document.getElementById('otp').value.replace(/ /g,'');
  if(!otp){showMsg('msg2','⚠️ Enter the OTP','error');return}
  setLoading('btn2',true,'Verify Code →');hideMsg('msg2');
  try{
    const r=await fetch('/verify_otp',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({otp})});
    const d=await r.json();
    if(d.ok){clearInterval(timerInterval);goStep(4)}
    else if(d.needs_2fa){clearInterval(timerInterval);goStep(3)}
    else if(d.expired){showMsg('msg2','⏰ OTP expired! Click Resend.','error');setLoading('btn2',false,'Verify Code →')}
    else{showMsg('msg2','❌ '+(d.error||'Invalid code'),'error');setLoading('btn2',false,'Verify Code →')}
  }catch(e){showMsg('msg2','❌ Network error.','error');setLoading('btn2',false,'Verify Code →')}
}
async function resendOTP(){
  document.getElementById('resendBtn').style.display='none';
  document.getElementById('timerText').textContent='⏳ Resending…';
  try{
    const r=await fetch('/resend_otp',{method:'POST'});const d=await r.json();
    if(d.ok){startTimer();showMsg('msg2','✅ New OTP sent!','success')}
    else{showMsg('msg2','❌ '+(d.error||'Resend failed'),'error');document.getElementById('resendBtn').style.display='block'}
  }catch(e){showMsg('msg2','❌ Network error.','error')}
}
async function verify2FA(){
  const pwd=document.getElementById('twofa').value;
  if(!pwd){showMsg('msg3','⚠️ Enter your password','error');return}
  setLoading('btn3',true,'Confirm Password →');hideMsg('msg3');
  try{
    const r=await fetch('/verify_2fa',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:pwd})});
    const d=await r.json();
    if(d.ok){goStep(4)}else{showMsg('msg3','❌ '+(d.error||'Wrong password'),'error');setLoading('btn3',false,'Confirm Password →')}
  }catch(e){showMsg('msg3','❌ Network error.','error');setLoading('btn3',false,'Confirm Password →')}
}
...
async def verify_otp_route(req):
    try:
        ...
        await c.sign_in(phone=state["phone"], code=otp, phone_code_hash=state["code_hash"])
        s = c.session.save()
        state["session"]   = s
...
def home_text():
    web_url = RENDER_URL if RENDER_URL else f"http://localhost:{PORT}"
    return (
        "```\n"
        "╔══════════════════════════════╗\n"
        "║   📡  AUTO SENDER  v2.3     ║\n"
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "cb_home":
        ...
        await q.message.reply_text(
            ...
            f"_Updates will come after every group._",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
...
def print_banner():
    console.print(Panel.fit(
        ...
        "[dim]Web OTP Login  •  Render Ready  •  Multi-Group[/dim]",
    ...
    t.add_row("Owner ID", str(OWNER_ID) if OWNER_ID else "⚠️  Set OWNER_ID env var")
...
async def run_all():
    ...
    if OWNER_ID == 0:
        console.print("[red]❌ OWNER_ID not set! Add it as env var.[/red]"); sys.exit(1)
    ...
    # Build web — all routes before freeze
    web_app = build_web_app()
    runner  = web.AppRunner(web_app)
    await runner.setup()
    site = web.TCPSite(runner, "0.0.0.0", PORT)
    await site.start()
    console.print(f"[cyan]🌐 Web login ready on port {PORT}[/cyan]")
